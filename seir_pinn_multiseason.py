...
def run_season(df, start_date, end_date, season_name,
    ...
    # --- PINN onset: first time R(t) > 1.0 ---
    rt_above = np.where(Rt_pred > 1.0)[0]
    if len(rt_above) > 0:
        ...
        for i in range(1, len(rt_above)):
            if rt_above[i] == rt_above[i - 1] + 1:
                consec += 1
                if consec >= 3:
                    pinn_onset_idx = rt_above[i - 2]  # start of the 3-point run
                    break
            else:
                consec = 1
        if pinn_onset_idx is None and len(rt_above) >= 3:
            pinn_onset_idx = rt_above[0]  # fallback if whole stretch is above 1
...
# ============================================================
SEASONS = [
    ("2014/15 winter", "2014-12-01", "2015-06-01"),
    ("2015/16 winter", "2015-12-01", "2016-06-01"),
    ("2016/17 winter", "2016-12-01", "2017-06-01"),
    ("2017/18 summer", "2017-06-01", "2018-04-01"),   # unusual summer wave
    ("2018/19 winter", "2018-12-01", "2019-06-01"),
    # 2020-2022: COVID NPIs — influenza near-zero, not trainable
    ("2023 summer",    "2023-04-01", "2023-10-01"),
    ("2023/24 winter", "2023-10-01", "2024-04-01"),
    ("2024/25 winter", "2024-10-01", "2025-04-01"),
]
...
if __name__ == "__main__":
    ...
    for season_name, start, end in SEASONS:
        res = run_season(df, start, end, season_name, n_epochs=15000, verbose=True)
        if res is not None:
            results.append(res)
